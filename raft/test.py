...
class RaftNode(raft_pb2_grpc.RaftServicer):
    def __init__(self, node_id, peers, election_timeout_range=(0.5, 0.65), heartbeat_interval=0.25):
    ...
    def AppendEntries(self, request, context):
        if request.term >= self.term:
            ...
            with self.lock:
                ...
                self.state = 'follower'
                self.reset_timeout()
                self.election_timeout = random.uniform(0.3, 0.45)
